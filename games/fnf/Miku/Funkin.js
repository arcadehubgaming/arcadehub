	...
	,update: function(elapsed) {
		...
		while(_g < _g1.length) {
			...
			if(touch.input.current == 2) {
				this.pressedSomething = true;
			}
		}
		if((flixel_FlxG.keys.justPressed.get_ANY() || this.pressedSomething) && this.dialogueStarted == true) {
...
FreeplayState.prototype = $extend(MusicBeatState.prototype,{
	...
	,update: function(elapsed) {
		...
		this.upP = PlayerSettings.player1.controls._upP.check();
		this.downP = PlayerSettings.player1.controls._downP.check();
		this.accepted = PlayerSettings.player1.controls._accept.check();
		this.scrollUp = false;
		...
		while(_g < _g1.length) {
			...
			if(Math.sqrt(dx * dx + dy * dy) >= 25) {
				if(swipe.startPosition.angleBetween(swipe.endPosition) >= -45 && swipe.startPosition.angleBetween(swipe.endPosition) <= 45) {
					this.scrollDown = true;
				}
				if(swipe.startPosition.angleBetween(swipe.endPosition) > -135 && swipe.startPosition.angleBetween(swipe.endPosition) < -45) {
					this.back = true;
				}
				if(swipe.startPosition.angleBetween(swipe.endPosition) > 45 && swipe.startPosition.angleBetween(swipe.endPosition) < 135) {
					this.scrollRight = true;
				}
				if(swipe.startPosition.angleBetween(swipe.endPosition) >= -180 && swipe.startPosition.angleBetween(swipe.endPosition) <= -135 || swipe.startPosition.angleBetween(swipe.endPosition) >= 135 && swipe.startPosition.angleBetween(swipe.endPosition) <= 180) {
					this.scrollUp = true;
				}
			} else {
				this.accept = true;
		...
		if(this.upP || this.scrollUp) {
			this.changeSelection(-1);
		}
		if(this.downP || this.scrollDown) {
			this.changeSelection(1);
		}
		if(PlayerSettings.player1.controls._leftP.check()) {
			this.changeDiff(-1);
		}
		if(PlayerSettings.player1.controls._rightP.check() || this.scrollRight) {
			this.changeDiff(1);
		}
		if(PlayerSettings.player1.controls._back.check() || this.back) {
		...
		if(this.accepted || this.accept) {
			...
			haxe_Log.trace(poop,{ fileName : "source/FreeplayState.hx", lineNumber : 243, className : "FreeplayState", methodName : "update"});
			...
			haxe_Log.trace("CUR WEEK" + PlayState.storyWeek,{ fileName : "source/FreeplayState.hx", lineNumber : 250, className : "FreeplayState", methodName : "update"});
...
var GameOverSubstate = function(x,y) {
	...
	this.bf.setGraphicSize(this.bf.get_width() * this.resizeFactor | 0);
	this.add(this.bf);
...
GameOverSubstate.prototype = $extend(MusicBeatSubstate.prototype,{
	...
	,update: function(elapsed) {
		...
		if(PlayerSettings.player1.controls._accept.check() || this.accept) {
			this.endBullshit();
		}
		if(PlayerSettings.player1.controls._back.check() || this.back) {
			var _this = flixel_FlxG.sound.music;
...
LoadReplayState.prototype = $extend(MusicBeatState.prototype,{
	...
	,create: function() {
		...
		haxe_Log.trace(this.controlsStrings,{ fileName : "source/LoadReplayState.hx", lineNumber : 44, className : "LoadReplayState", methodName : "create"});
	...
	,update: function(elapsed) {
		...
		if(PlayerSettings.player1.controls._upP.check() || this.scrollUp) {
			this.changeSelection(-1);
		}
		if(PlayerSettings.player1.controls._downP.check() || this.scrollDown) {
			this.changeSelection(1);
		}
		if((PlayerSettings.player1.controls._accept.check() || this.accept) && this.grpControls.members[this.curSelected].text != "No Replays...") {
			haxe_Log.trace("loading " + this.actualNames[this.curSelected],{ fileName : "source/LoadReplayState.hx", lineNumber : 181, className : "LoadReplayState", methodName : "update"});
						...
						if(_gthis.curSelected != spr.ID) {
							...
							flixel_effects_FlxFlicker.flicker(spr,1,0.06,false,false,function(flick) {
								...
								case "freeplay":
									...
									haxe_Log.trace("Freeplay Menu Selected",{ fileName : "source/MainMenuState.hx", lineNumber : 208, className : "MainMenuState", methodName : "update"});
									break;
								case "options":
									var nextState = new OptionsMenu();
									if(flixel_FlxG.game._state.switchTo(nextState)) {
								...
								case "story mode":
									...
									haxe_Log.trace("Story Menu Selected",{ fileName : "source/MainMenuState.hx", lineNumber : 204, className : "MainMenuState", methodName : "update"});
									break;
								}
							});
						}
